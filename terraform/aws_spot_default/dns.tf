...
resource "cloudflare_record" "cloudflare_code_dns" {
  count  = "${var.dns_service == "cloudflare" && var.aws_server_count > 0 ? var.aws_server_count:0}"
  zone_id = var.cloudflare_zone_id
  name   = "code-${element(aws_spot_instance_request.lab-server.*.tags.Name, count.index)}"
  value  = aws_eip.haproxy-eip.public_ip
  type   = "A"
  ttl = 600
}
...
resource "cloudflare_record" "cloudflare_rancher_dns" {
  count  = "${var.dns_service == "cloudflare" && var.aws_server_count > 0 ? var.aws_server_count:0}"
  zone_id = var.cloudflare_zone_id
  name   = "rancher-${element(aws_spot_instance_request.lab-server.*.tags.Name, count.index)}"
  value  = aws_eip.haproxy-eip.public_ip
  type   = "A"
  ttl = 600
}
...
resource "cloudflare_record" "cloudflare_portainer_dns" {
  count  = "${var.dns_service == "cloudflare" && var.aws_server_count > 0 ? var.aws_server_count:0}"
  zone_id = var.cloudflare_zone_id
  name   = "portainer-${element(aws_spot_instance_request.lab-server.*.tags.Name, count.index)}"
  value  = aws_eip.haproxy-eip.public_ip
  type   = "A"
  ttl = 600
}
